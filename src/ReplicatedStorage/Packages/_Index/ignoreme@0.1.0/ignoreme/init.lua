...
function Component:_setup()
	...
	local startedGlobalHB = false

	local function StartComponent(component)
		if not startedGlobalHB then
			if type(self["GlobalHeartbeatUpdate"]) == "function" then
				self[KEY_TROVE]:Add(RunService.Heartbeat:Connect(function(dt)
					for i, v in self[KEY_COMPONENTS] do
						v:GlobalHeartbeatUpdate(dt)
					end
				end))
			end
			startedGlobalHB = true
		end
