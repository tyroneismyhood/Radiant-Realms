...
alignmentDetails["right"] = {
	...
	getOffset = function()
		...
		if
			(checkTopbarEnabled() or VRService.VREnabled)
			and (
				starterGui:GetCoreGuiEnabled(Enum.CoreGuiType.PlayerList)
				or starterGui:GetCoreGuiEnabled(Enum.CoreGuiType.Backpack)
				or (not isR6 and starterGui:GetCoreGuiEnabled(Enum.CoreGuiType.EmotesMenu))
			)
		then
			offset += 48
		end
	...
	icon.selected:Connect(function()
		...
		for _, otherIcon in allIcons do
			if
				...
				and otherIcon ~= icon
				and otherIcon.deselectWhenOtherIconSelected
				and otherIcon:getToggleState() == "selected"
			then
...
function IconController.setTopbarEnabled(bool, forceBool)
	...
	if IconController.controllerModeEnabled then
		if bool then
			if TopbarPlusGui.TopbarContainer.Visible or forceTopbarDisabled or menuOpen or not topbarEnabledAccountingForMimic then
				return
			end
			if forceBool then
...
function IconController._enableControllerModeForIcon(icon, bool)
	...
	if bool then
		...
		icon:set(
			"iconSize",
			UDim2.new(0, currentSizeDeselected.X.Offset * scaleMultiplier, 0, currentSizeDeselected.Y.Offset * scaleMultiplier),
			"deselected",
			"controllerMode"
		)
		icon:set(
			"iconSize",
			UDim2.new(0, currentSizeSelected.X.Offset * scaleMultiplier, 0, currentSizeSelected.Y.Offset * scaleMultiplier),
			"selected",
			"controllerMode"
		)
			...
			:give(function(icon)
				...
				local function listenToHealth(character)
					...
					local function updateHealthBar()
						...
						if
							healthInterval == 1
							or IconController.healthbarDisabled
							or (firstTimeEnabling and realHealthbarEnabled == false)
						then
						...
						local colorInterval = (colorIntervalAbsolute > 1 and 1)
							or (colorIntervalAbsolute < 0 and 0)
							or colorIntervalAbsolute
...
coroutine.wrap(function()
	...
	for alignment, detail in alignmentDetails do
		if alignment ~= "mid" then
			...
			local overflowIcon = Icon.new():setProperty("internalIcon", true):setImage(6069276526):setName(overflowName):setEnabled(false)
	...
	task.defer(function()
		...
		while true do
			success, enabledForUser = pcall(function()
	...
	if not isStudio then
		...
		if game.CreatorType == Enum.CreatorType.Group then
			local success, ownerInfo = pcall(function()
...
coroutine.wrap(function()
	local chatScript = players.LocalPlayer.PlayerScripts:WaitForChild("ChatScript", 4)
		or game:GetService("Chat"):WaitForChild("ChatScript", 4)
