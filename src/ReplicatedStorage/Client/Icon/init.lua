...
function Icon.new()
	...
	self._settings = {
		...
		toggleable = {
			...
			["forcedIconSizeY"] = {},
			["iconSize"] = {
				callSignals = { self.updated },
				callMethods = { self._updateIconSize },
		...
		["dropdown"] = function(settingName, instance, propertyName, value)
			...
			if #self.dropdownIcons > 0 and isOpen and hidePlayerlist then
				if starterGui:GetCoreGuiEnabled(Enum.CoreGuiType.PlayerList) then
					IconController._bringBackPlayerlist = (IconController._bringBackPlayerlist and IconController._bringBackPlayerlist + 1)
						or 1
...
function Icon:set(settingName, value, iconState, setAdditional)
	...
	if iconState == "hovering" then
		...
		if
			not self._updateAfterSettingAll
			and settingDetail.instanceNames
			and (currentToggleState == toggleState or toggleState == nil)
		then
			local ignoreTweenAction = (settingName == "iconSize" and previousValue and previousValue.X.Scale == 1)
			local tweenInfo = (settingDetail.tweenAction and not ignoreTweenAction and self:get(settingDetail.tweenAction))
				or TweenInfo.new(0)
...
function Icon:get(settingName, iconState, getAdditional)
	...
	if settingType == "toggleable" then
		...
		if additionalValueToReturn == nil then
			additionalValueToReturn = type(getAdditional) == "string"
				and settingDetail.additionalValues[getAdditional .. "_" .. toggleState]
		end
...
function Icon:_update(settingName, toggleState, customTweenInfo)
	...
	local tweenInfo = customTweenInfo
		or (settingDetail.tweenAction and settingDetail.tweenAction ~= "" and self:get(settingDetail.tweenAction))
		or self:get("toggleTransitionInfo")
		or TweenInfo.new(0.15)
...
function Icon:_getContentText(text)
	...
	local translatedContentText = typeof(self.instances.iconLabel) == "Instance"
		and IconController.translator:Translate(self.instances.iconLabel, textToTranslate)
...
function Icon:_updateCaptionSize()
	...
	if iconSize and labelFont then
		...
		if isVisible then
			...
			local textWidth =
				textService:GetTextSize(iconContentText, captionLabelHeight, labelFont, Vector2.new(10000, captionLabelHeight)).X
...
function Icon:displayCaption(bool)
	...
	local newPos = UDim2.new(
		0,
		iconContainer.AbsolutePosition.X + iconContainer.AbsoluteSize.X / 2 - captionContainer.AbsoluteSize.X / 2,
		0,
		iconContainer.AbsolutePosition.Y + (iconContainer.AbsoluteSize.Y * 2) + yOffset
	)
...
function Icon:_updateDropdown()
	...
	local additionalOffset = (dropdownFrame.VerticalScrollBarPosition == Enum.VerticalScrollBarPosition.Right and newThickness)
		or -newThickness
...
function Icon:_updateMenu()
	...
	local sortFunc = (direction == "right" and function(a, b)
		return a:get("order") < b:get("order")
	end) or function(a, b)
		return a:get("order") > b:get("order")
	end
